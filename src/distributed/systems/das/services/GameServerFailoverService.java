...
import distributed.systems.das.BackupGameServer;
import distributed.systems.das.BackupRequestHandlingServer;
import distributed.systems.das.GameState;
import distributed.systems.das.common.MessageType;

public class GameServerFailoverService implements Runnable{
	
	private BackupGameServer serverState;
	Thread t;
	
	public GameServerFailoverService(BackupGameServer backupRequestHandlingServer) {
		...
		while(GameState.getRunningState()) {
			if(serverState.getTimeSinceHeartbeat()!= null && !serverState.getTimeSinceHeartbeat().isEmpty()) {
				...
				serversFailed.forEach(s -> {
					...
					serverState.processServerFailure();
				});
			}
		}
	}
